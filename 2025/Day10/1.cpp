...
// each listed indicator light either turns on (if it was off) or turns off (if
// it was on). You have to push each button an integer number of times; there's
// no such thing as "0.5 presses" (nor can you push a button a negative number
// of times).
//
// So, a button wiring schematic like (0,3,4) means that each time you push that
// button, the first, fourth, and fifth indicator lights would all toggle
...
//     presses. You could press (1,3) once, (2,3) once, and (0,1) twice, a total
//     of 4 button presses. You could press all of the buttons except (1,3) once
//     each, a total of 5 button presses.
//
...
string splitString(string str, char firstChar, char secondChar) {
  size_t startIndex = str.find(firstChar);
  size_t endIndex = str.find(secondChar, startIndex);
...
int main() {
  ...
  for (int i = 0; i < no_of_line; i++) {
    string indicators = splitString(lines[i], '-', '-');
    string buttons = splitString(lines[i], '(', ')');
        cout<<indicators<<" "<<buttons<<endl;
  }
}
//Stuck
